...
func WithClaims(ctx context.Context, claims *types.AegisAttestedClaims, unifiedJSON []byte) context.Context {
	if claims != nil {
...
func FromContext(ctx context.Context, _ ...bool) (*types.AegisAttestedClaims, []byte) {
	claims, _ := ctx.Value(attestedClaimsKey).(*types.AegisAttestedClaims)
...
func WithSovereignAttestation(ctx context.Context, sa *types.AegisSovereignAttestation) context.Context {
...
func FromSovereignAttestation(ctx context.Context) *types.SovereignAttestation {
	...
	if ok {
		...
		if len(values) > 0 {
			...
			sa := &types.AegisSovereignAttestation{}
